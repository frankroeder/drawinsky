...
class EncodeFn(beam.DoFn):
    def process(self, element, img_size=512, lw=3):
...
def run(argv=None):
  ...
  input = p | 'Read files' >> ReadFromText("../test.ndjson")
  jsons = input | 'Load into JSON' >> beam.Map(lambda x: json.loads(x))
  b64 = jsons | 'Convert to images' >> beam.ParDo(EncodeFn())
  b64 | 'Save images' >> beam.ParDo(WriteToSeparateFiles('./images/' + known_args.output))
